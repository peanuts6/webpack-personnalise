...
function prepareResult(content, opts) {
    ...
    if (typeof content === "string" && content.indexOf("module.exports") === 0)
        return content
...
function resolve(content, plugin) {
    ...
    try {
        ...
        result = resultCustomizer(
            result,
            plugin.resourcePath,
            templateOpts,
            plugin.resourceQuery
        )
    } catch (e) {
...
function parseQuery(query) {
    return JSON.parse(
        JSON.stringify(queryString.parse(query, {arrayFormat: "bracket"}))
    )
}
